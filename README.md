...
4. Retrieve the Rendered Preview Image
Fetch the diagnostic preview image generated by the background Python worker:
```
curl https://api-21f-3000.ny1.zerops.app/api/studies/<study_uid>/preview \
  --output preview.png
```
Expected Result: A clean, high-resolution preview.png saved to your current working directory.

5. Verify Ingestion Validation Guardrails
Test the file verification middleware by uploading a non-DICOM dummy file:
```
curl -X POST -F "file=@README.md" \
  https://api-21f-3000.ny1.zerops.app/api/dicom-ingest
```
Expected Result: The upload is immediately blocked and returns a 400 Bad Request with an invalid header error (the pipeline checks for the standard DICM signature bytes).
